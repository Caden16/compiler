...
def drawTable(header,subHeader,component,p_width = 80,ifCenter = 1):   # 最好把component内容转化为字符串list
	global width
	global subWidth
	global div
	width = p_width
...
def drawNoCenter(content): # 左对齐
	...
	for i in range(localsDiv):
		drawSymbol('|',1)
		print(str(content[i]).ljust(localsWidth - containChinese(content[i])),end = "")
	drawSymbol('|',1)
	print()	 #输完一个表项后换行	
